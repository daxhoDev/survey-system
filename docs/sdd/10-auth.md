...
- **AUTH-32** `[implemented]` Every email that reaches the API (login, invitations, `create-user`) is converted to lowercase by the backend (`utils/email.ts`) before it is compared or stored, so the checks of AUTH-10 and the login ignore case. The database stores what the backend writes; it has no case-insensitive constraint or index.
...
- **AUTH-26** `[implemented]` `DELETE /api/v1/invitations/:id` — authenticated. Revokes a pending invitation (`revoked_at = now`) → `204`. An id that is not a UUID → `422` (API-34); unknown id → `404 Not found` ("The requested invitation doesn't exist"); an invitation that is not pending → `409 Conflict` ("This invitation is no longer pending").
